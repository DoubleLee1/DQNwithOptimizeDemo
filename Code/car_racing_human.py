...
class CarRacing(gym.Env, EzPickle):
    ...
    def render_indicators(self, W, H):
        ...
        # vertical_ind(5, 0.05*true_speed, (1, 1, 1))  # show speed
        # vertical_ind(7, 0.01*self.car.wheels[0].omega, (0.0,0,1)) # ABS sensors
        # vertical_ind(8, 0.01*self.car.wheels[1].omega, (0.0,0,1))
        # vertical_ind(9, 0.01*self.car.wheels[2].omega, (0.2,0,1))
        # vertical_ind(10,0.01*self.car.wheels[3].omega, (0.2,0,1))
        horiz_ind(5, 0.1*true_speed, (1, 1, 1))
...
if __name__=="__main__":
    ...
    # set a address to save data
    file = 'F:/DQfD/Data/1.p'
    data = []
    ...
    env = CarRacing()
    env.render()
    # print('The position is', env.render().car.hull.position[0], env.render().car.hull.position[1])
    ...
    while isopen:
        ...
        for i in range(60):
            picture, r, done, info = env.step(np.array([0.0, 0.0, 0.0]))
            state_pre = np.array([0])
            # env.render()

        while True:
            s, r, done, info = env.step(a)
            s = cut_state(s)        # # cut the button black line
            restart = detector(s)   # detect if out the track
            if restart: r = -50.    # if out the track give a punish
            total_reward += r
            if done: print("step {} total_reward {:+0.2f}".format(steps, total_reward))
                #import matplotlib.pyplot as plt
                #plt.imshow(s)
            ...
            # # save data
            # if steps % 2 == 0:
            #     gray = cvtColor(s, COLOR_BGR2GRAY)
            #     gray_ = cvtColor(s_, COLOR_BGR2GRAY)
            #     aciton = np.array(a, dtype='float32')
            #     data.append([gray_, aciton, np.array([r], dtype='float32'), gray])
            # else:
            #     pass
